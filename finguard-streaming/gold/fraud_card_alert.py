...
def fraud_card_alert() -> DataFrame:
    ...
    transactions_with_watermark = transactions.withWatermark(
        "transaction_timestamp", "5 minutes"
    )
    fraud_watchlist_with_watermark = fraud_watchlist.withWatermark(
        "effective_from", "5 minutes"
    )
    ...
    fraud_detected = (
        transactions_with_watermark.join(
            ...
            == fraud_watchlist_with_watermark.entity_id,
            "inner",
        )
        .join(
            customers,
        ...
        .select(
            ...
            F.concat_ws(
                "-", F.lit("FRAUD"), F.col("transaction_id"), F.col("watchlist_id")
            ).alias("alert_id"),
            ...
            F.concat_ws(" ", customers.first_name, customers.last_name).alias(
                "customer_name"
            ),
            transactions_with_watermark.card_number,
